...
public class EFDTBoost extends VFDT{
	...
	public interface EFDTNode {
		...
		public void learnFromInstance(Instance inst, EFDTBoost ht, EFDTSplitNode parent, int parentBranch);
	...
	public class EFDTSplitNode extends SplitNode implements EFDTNode{
		...
		public AttributeSplitSuggestion[] getBestSplitSuggestions(
				SplitCriterion criterion, EFDTBoost ht) {
		...
		public void learnFromInstance(Instance inst, EFDTBoost ht, EFDTSplitNode parent, int parentBranch) {
			...
			if (child != null) {
				...
				((EFDTNode) child).learnFromInstance(weightedInst, ht, this, childBranch);
			}
		...
		protected void reEvaluateBestSplit(EFDTSplitNode node, EFDTSplitNode parent,
			...
			SplitCriterion splitCriterion = (SplitCriterion) getPreparedClassOption(EFDTBoost.this.splitCriterionOption);
			double hoeffdingBound = computeHoeffdingBound(splitCriterion.getRangeOfMerit(node.getClassDistributionAtTimeOfCreation()),
					EFDTBoost.this.splitConfidenceOption.getValue(), node.observedClassDistribution.sumOfValues());

			// get best split suggestions
			AttributeSplitSuggestion[] bestSplitSuggestions = node.getBestSplitSuggestions(splitCriterion, EFDTBoost.this);
			...
			double tieThreshold = EFDTBoost.this.tieThresholdOption.getValue();
			...
			if (deltaG > hoeffdingBound
				...
				if(splitDecision.splitTest == null){
					...
					node.killSubtree(EFDTBoost.this);
					...
					if (node.splitTest == splitDecision.splitTest
						...
						this.killSubtree(EFDTBoost.this);
						...
						EFDTBoost.this.activeLeafNodeCount--;
						EFDTBoost.this.decisionNodeCount++;
						EFDTBoost.this.activeLeafNodeCount += splitDecision.numSplits();
					...
					if (parent == null) {
						...
						EFDTBoost.this.treeRoot = newSplit;
					} else {
